...
;	BANK1							;Troca para o banco de memória RAM No 1 para conseguir mais
									;espaço livre.
									;Acho que eu não preciso trocar de banco já que configurei o IRP
									;e só estou acessando memória fora dos shadows usando FSR/INDF.
...
ESCREVE_NA_LINHA
	...
	movwf		PCLATH					;alcançar uma página de memória de programa
										;diferente da atual.
...
;Para saber qual o caractere recebido, vou simplesmente criar uma tabela ASCII (de 0x00 até 0x7F)
	ORG		0x400						;Garanto que a soma sempre dará dentro do
										;intervalo de 8 bits do PCL e este corresponderá
										;a página correta da memória.
TABELA_ASCII
	...
	goto	ABRE_PARENT  	;(0x28)	
	goto	FECHA_PARENT  	;(0x29)	
	goto	ASTERISCO	  	;(0x2A)	
	goto	MAIS		  	;(0x2B)	
...
;primeiro pedaço ->endereco_inicial+ordem_no_buffer+pulo
;endereco_inicial:OK
;ordem_no_buffer: CONT_BUFFER2
;pulo:usando um addlw
;ENDERECO_LETRA->endereco_inicial+ordem no buffer, só falta o pulo de (0x06 pra cada pedaço)!
...
LETRA_A
	...
	movlw	B'00000111'
	movwf	INDF				;Escreve na memória de imagem o primeiro pedaço do caractere.
