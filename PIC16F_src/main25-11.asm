...
;NÃO ESQUECER QUE TODAS ESSAS VARIÁVEIS SERÃO DE 8BITS
	...
	CBLOCK		0xA0				;ENDEREÇO INICIAL DA MEMÓRIA DO BANCO 1 (0xA0 ATÉ 0xEF)
				...
				BUFFER1				;Buffers utilizados para guardar os caracteres recebidos após um '@'.
				BUFFER2
				BUFFER3
				BUFFER4
				BUFFER5
...
#DEFINE			LED_SEROK	PORTB,4		;Indica que tudo está correndo bem.
...
;
;A utilizacao de um buffer de comandos faz com que a serial nao prejudique a geracao
;dos pulsos de sincronismo.
;Apos o buffer de comando ter sido finalizado com um CR, fica liberado o processamento
;utilizando a tela em branco (campo par).
...
;QUERO LIGAR ALGUM LED (ou led's) QUANDO RECEBER UM @ E DEPOIS
;DESLIGAR OU LIGAR OUTROS LEDs QUANDO RECEBER O RETURN. TAMBEM PRECISO
;DE UM LED INDICANDO QUE A LINHA JAH ESTA CHEIA.

;No caso do return (CR), ainda quero verificar se sobram espacos na linha para preenche-la com zeros.
...
;Caso receba um CR, preencho o buffer com espaços em branco.
;Caso receba um ESC, apago tudo e seto ARROBA com zero.
