...
;NÃO ESQUECER QUE TODAS ESSAS VARIÁVEIS SERÃO DE 8BITS
	...
	CBLOCK		0x70				;ENDEREÇO INICIAL DA MEMÓRIA DE USUÁRIO (0x70 ATÉ 0x7F)
				...
				SALVA_FSR2			;Usado para salvar o conteudo do FSR durante a execução dos comandos (0x7C)
...
;########### APÓS EXECUTAR OS COMANDOS NÃO POSSO ESQUECER DE VOLTAR PRO SEM_SINC_H!!!!!
	...
	movfw		FSR					;Salva o conteúdo do FSR para não correr o risco
	movwf		SALVA_FSR2			;de ocorrer uma interrupção da serial no meio deste código.
	...
	btfsc		STATUS,2			;Testa se o Z mudou para 1.
	goto		ESCREVE_NA_L1		;Se o Z foi para 1, recebi o '1'.
	xorlw		UM					;Restaura
...
ESCREVE_NA_L1
	...
	ORG		0X400				;Para garantir o PCL(7-0) 0x00 e assim não ter problema
								;com a troca do PCLATH(12-8) quando fizer um addwf PCL,F.
TABELA_CARACTERES
	addwf	PCL,F				;No W vai estar armazenado o 'salto.
								...
								;O datasheet chama isso de 'Computed Goto'.
