...
;NÃO ESQUECER QUE TODAS ESSAS VARIÁVEIS SERÃO DE 8BITS
	...
	CBLOCK		0x20				;ENDEREÇO INICIAL DA MEMÓRIA DE USUÁRIO
				...
				CONT_VERT			;Usada para contar o numero de linhas horizontais que foram desenhadas (0x23)
				P_EQUA				;Conta os pulsos de equalizacao(0x24)
				P_EQUAF				;Conta os ultimos pulsos de equalizacao(0x25)
				P_VSINC				;Conta os pulsos de sincronismo invertido(0x26)
				FLAGS				;Usado para as flags do programa (0x27)
...
#DEFINE			ODD				FLAGS,1			;Indica se estou no campo par ou impar

#DEFINE			SINC_V			FLAGS,2			;Sinaliza quando fazer o sincronismo

#DEFINE			FIM_SINC_VERT	FLAGS,3			;Sinaliza que eh a ultima equalizacao
...
;NOP?? 
;Gera um monte de... nop! Conforme os ??
NOP50	MACRO
					nop
					nop
					nop
...
;DE "W" E "STATUS" PARA RECUPERAÇÃO FUTURA
	...
	;Testa para verificar se este eh um pulso de sincronismo 
	btfss		SINC_V

	;Inicia a borda preta do sincronismo para economizar tempo
	;Ela tera 1.4uS de extensao
	bcf			saida
...
INT_TIMER0 ;Ateh aqui foram 6 ciclos
	...
	;Ateh aqui consigo 1.4uS de preto 
	;referente ao inicio do sincronismo

	;Testa para verificar se este eh um pulso de sincronismo 
;	btfsc		SINC_V
;	goto		SINC_VERT			;Pula para tratar sincronismos verticais

	bcf			sincronismo
	;Inicio do pulso de 4.7uS no nivel de sincronismo
	
	;Estou aproveitando o tempo morto aqui para contar o numero de linhas
	;verticais geradas ateh o momento
	INCFSZ		CONT_VERT
	goto		CONTINUA_SINC		;Ainda nao passaram 242 linhas

	bsf			SINC_V				;Passaram 242 linhas, ativo o 
									;sincronismo vertical para a proxima linha

CONTINUA_SINC
	bsf			SINC_H				;Setando esse bit eu garanto que apos o
									;sincronismo podera ser mostrada a imagem
	NOP10
	NOP10
	...
	;Aqui eu utilizaria 24 NOP10, mas como sao gastos 8 ciclos para
...
EQUALIZACAO
	;P1
	;Inicio do pulso de 4.7uS no nivel de sincronismo
	INCFSZ		P_EQUA					;Testa se jah foram os 3 pulsos de equalizacao
	goto		CONT_EQ
...
CONT_EQ
	...
	;Gera o pulso de 26.95uS no nivel de preto da equalizacao 
	bsf			sincronismo
	NOP50
	NOP50
	NOP10
	...
	nop
	nop
	INCFSZ		P_EQUAF					;Testa se jah foram os 3 pulsos de equalizacao
	goto		SAI_INT

	bcf			SINC_V					;Libera os pulsos de sincronismo
	goto		SAI_INT
	
SINC_INVERTIDO
	;P1
	;Inicio do pulso de 4.7uS no nivel de sincronismo
	INCFSZ		P_VSINC					;Testa se jah foram os 3 pulsos
	goto		CONT_SI
	
	movlw		.252					;Inicializa o P_EQUAL
	movwf		P_EQUA
	movlw		.253
	movwf		P_EQUAF					;Conta os 3 ultimos pulsos

CONT_SI
	;Gera o pulso de 26.95uS no nivel de sinc do sincronismo vertical
	bcf			sincronismo
	NOP50
	NOP50
	NOP10
	...
	;Inicio do pulso de 4.7uS no nivel de preto
	bsf			sincronismo
	NOP10
	NOP10
	nop
...
;****************************************************************************
	...
	movlw		.13
	movwf		CONT_VERT			;Inicializa para contar 242 linhas

	movlw		.252
	movwf		P_EQUA				;Inicializa o contador de pulsos de equalizacao
	
	movlw		.0
	movwf		P_EQUAF				;Usado para os ultimos 3 pulsos de equalizacao

	movlw		.253
	movwf		P_VSINC				;Inicializa o contador de pulsos de sinc invertido

	bcf			SINC_H				;Para que a imagem soh comece depois do
									;TIMER0 ter estourado uma vez

	bcf			FIM_SINC_VERT		;Indica o ultimo trem de pulsos de equalizacao
...
MAIN
	...
	movlw		B'00000000'		;Carrega a memória RAM com esse valor
	movwf		0x40			;para eu poder testar as rotinas de leitura/escrita de bit
	movlw		B'00000000'		;no pino definido por saida.'
	movwf		0x41	
	movlw		B'10101010'		
	movwf		0x42
	movlw		B'10101010'
